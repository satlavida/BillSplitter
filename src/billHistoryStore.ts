...
interface BillHistoryStoreState {
  version: string;
...
interface BillHistoryStoreActions {
  ...
  exportBills: () => string;
  importBills: (jsonString: string) => ImportResult;
  clearHistory: () => void;
}

type BillHistoryStore = BillHistoryStoreState & BillHistoryStoreActions;

// Initial state
const initialState: BillHistoryStoreState = {
...
const useBillHistoryStore = create<BillHistoryStore>()(
  persist(
    (set, get) => ({
      // State
      ...initialState,
      ...
      addBill: (billData) =>
        set((state) => {
          ...
          const newBill = {
            ...
            data: { ...billData, billId } as BillHistoryEntry['data'], // Include billId in the data
            isCurrent: true,
            version: BILL_HISTORY_VERSION,
          };
      ...
      saveBill: (billData) =>
        set((state) => {
          ...
          let updatedBills: BillHistoryEntry[] = [];
          let isNewBill = false;
          ...
          const dataWithId = { ...billData, billId } as BillHistoryEntry['data'];
          ...
          return {
            bills: updatedBills,
            currentBillId: billId as string,
          };
        }),
      ...
      deleteBill: (billId) =>
        set((state) => {
          ...
          const currentBillId = state.currentBillId === billId ? null : state.currentBillId;
      ...
      importBills: (jsonString) => {
        try {
          ...
          set((state) => {
            ...
            const newBills = imported.bills.filter((bill: BillHistoryEntry) => !existingIds.has(bill.id));
    ...
    }),
    {
      ...
      merge: (persistedState, currentState) => {
        ...
        const result = BillHistoryStateSchema.safeParse(persistedState);
        if (!result.success) {
          console.error('Failed to parse persisted billHistory state, falling back to defaults:', result.error);
          return currentState;
        }
