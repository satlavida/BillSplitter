...
import useBillStore from 'src/billStore';
...
interface PassAndSplitActions {
  activate: () => void;
  deactivate: () => void;
  selectPerson: (personId: string) => void;
  assignCurrentItem: () => void;
  skipCurrentItem: () => void;
  completeCurrentPerson: () => void;
  resetCurrentPerson: () => void;
  commitAssignmentsForPerson: (personId: string) => void;
  commitAssignments: () => void;
  addNewPerson: (name: string) => void;
}

type PassAndSplitStore = PassAndSplitState & PassAndSplitActions;

// Define the Pass and Split store
const usePassAndSplitStore = create<PassAndSplitStore>()(
  devtools(
    (set, get) => ({
      // State
      ...
      assignCurrentItem: () => {
        ...
        set((state) => ({
          ...
          pendingAssignments: {
            ...state.pendingAssignments,
            [currentPersonId]: [...state.pendingAssignments[currentPersonId], currentItemId],
          },
      ...
      commitAssignmentsForPerson: (personId) => {
        ...
        billItems.forEach((item) => {
          ...
          // NOTE: consumedBy entries are normalized to {personId, value} objects
          // by BillStateSchema on hydration (see billStore.ts's persist `merge`),
          // but this defensive string-shape check is kept as a safety net for
          // any in-memory state that predates that normalization running.
          // TODO(phase2): remove once all state is guaranteed schema-normalized on load.
          const existingConsumers = item.consumedBy.filter((consumer) => {
            return typeof consumer === 'string' ? consumer !== personId : consumer.personId !== personId;
          });

          // Get clean list of person IDs
          const personIds = existingConsumers.map((consumer) =>
          ...
          const wasPersonPreviouslyAssigned = item.consumedBy.some((consumer) =>
            typeof consumer === 'string' ? consumer === personId : consumer.personId === personId
          );
