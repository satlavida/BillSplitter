...
interface PassAndSplitProps {
  onClose: () => void;
}

const PassAndSplit = ({ onClose }: PassAndSplitProps) => {
  ...
  if (people.length === 0) {
    return (
      ...
      >
        <div
          className={`relative w-11/12 max-w-md p-6 rounded-lg shadow-lg
                       dark:bg-gray-800 bg-white`}
        >
          <h3 className="text-xl font-bold mb-4">No People Added</h3>
          <p className="mb-4">You need to add people to the bill before using Pass and Split mode.</p>
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="min-h-screen px-4 flex items-center justify-center">
        ...
        <div
          className={`relative w-full max-w-md p-0 rounded-xl shadow-2xl transition-all
        ...
        >
          ...
          >
            ...
            <button onClick={handleClose} className={`rounded-full p-1 dark:hover:bg-gray-700 hover:bg-gray-200`}>
              {/* X icon */}
          ...
          {/* Modal Content */}
          <div className="flex flex-col">{isActive && <PassAndSplitController />}</div>
        </div>
      </div>
    </div>
  );
