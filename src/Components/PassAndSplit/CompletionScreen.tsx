...
const CompletionScreen = () => {
  ...
  const selectedItemIds = (currentPersonId && pendingAssignments[currentPersonId]) || [];
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 text-center">
        ...
        <p className={`text-sm dark:text-gray-300 text-gray-500`}>
          {selectedItems.length === 0 ? "You didn't select any items" : `You selected ${selectedItems.length} item${selectedItems.length !== 1 ? 's' : ''}`}
        </p>
      </div>
            ...
            {selectedItems.map((item, index) => (
              <li
                ...
                className={`
                  ...
                  ${index !== selectedItems.length - 1 ? `border-b dark:border-gray-700 border-gray-200` : ''}
              ...
              >
                ...
                <span className={`text-sm dark:text-gray-400 text-gray-500`}>× {item.quantity}</span>
              </li>
            ))}
          </ul>
        )}
