    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 text-center">
        ...
        <p className={`text-sm dark:text-gray-300 text-gray-500`}>
          {allCompleted ? 'Everyone has completed their selections!' : 'Select your name to choose your items'}
        </p>
      </div>
            ...
            return (
              <button
                ...
                className={`
                  ...
                  h-24 transition-all ${
                    isCompleted ? `opacity-50 dark:bg-gray-700 bg-gray-100` : `dark:bg-blue-700 dark:hover:bg-blue-600 bg-blue-100 hover:bg-blue-200`
                  }
                `}
              >
                ...
                <div
                  ...
                  ${isCompleted ? `dark:bg-gray-600 bg-gray-300` : `dark:bg-blue-500 bg-blue-500 text-white`}
                `}
                >
                  {person.name.charAt(0).toUpperCase()}
                </div>

                <span className="text-sm font-medium truncate max-w-full">{person.name}</span>

                {isCompleted && <span className={`text-xs mt-1 dark:text-green-400 text-green-600`}>✓ Completed</span>}
              </button>
            );
          })}
        </div>
