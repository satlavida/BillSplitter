...
const BillTotalsSummary = memo(({ subtotal, taxAmount, grandTotal, className = '', formatCurrency }: BillTotalsSummaryProps) => {
  return (
    <div className={`p-4 bg-zinc-50 dark:bg-zinc-700 rounded-lg border border-zinc-200 dark:border-zinc-600 transition-colors ${className}`}>
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-zinc-800 dark:text-white transition-colors">{formatCurrency(subtotal)}</span>
      </div>
      <div className="flex justify-between items-center mb-2">
        <span className="font-medium text-zinc-800 dark:text-white transition-colors">Tax Amount:</span>
        <span className="text-zinc-800 dark:text-white transition-colors">{formatCurrency(taxAmount)}</span>
      </div>
