...
interface PercentageSplitInputProps {
  ...
  onSave: (percentages: Allocation[]) => void;
  onCancel: () => void;
}

const PercentageSplitInput = ({ people, allocations, onSave, onCancel }: PercentageSplitInputProps) => {
  // Initialize state with current allocations or default values
  const [percentages, setPercentages] = useState<Allocation[]>(() => {
  ...
  const handlePercentageChange = (personId: string, newValue: string) => {
