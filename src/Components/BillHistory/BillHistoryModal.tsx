...
import type { BillHistoryEntry } from '../../schemas/billHistory.schema';
...
interface BillHistoryItemProps {
  bill: BillHistoryEntry;
  onLoad: (billId: string) => void;
  onDelete: (billId: string) => void;
}

// BillHistoryItem component
const BillHistoryItem = ({ bill, onLoad, onDelete }: BillHistoryItemProps) => {
  return (
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-medium text-zinc-800 dark:text-white transition-colors">
            ...
            <span className="text-xs ml-2 text-zinc-500 dark:text-zinc-400 transition-colors">#{bill.id}</span>
          </h3>
          <p className="text-sm text-zinc-600 dark:text-zinc-400 transition-colors">{formatDate(bill.date)}</p>
        </div>
        <div className="flex space-x-2">
          <Button variant="secondary" size="sm" onClick={() => onLoad(bill.id)} disabled={bill.isCurrent}>
            {bill.isCurrent ? 'Current' : 'Load'}
          </Button>
          <Button variant="danger" size="sm" onClick={() => onDelete(bill.id)}>
            Delete
          </Button>
        </div>
      </div>
...
const BillHistoryModal = ({ isOpen, onClose }: BillHistoryModalProps) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [importError, setImportError] = useState<string | null>(null);
  const [importSuccess, setImportSuccess] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [sortMethod, setSortMethod] = useState<SortMethod>('date'); // 'date' or 'name'
  ...
  const { bills, deleteBill, clearHistory, setCurrentBill, exportBills, importBills } = useBillHistoryStore(
    useShallow((state) => ({
      bills: state.bills,
  ...
  const filteredAndSortedBills = useMemo(() => {
    ...
    const filtered = bills.filter((bill) => (bill.title || 'Untitled Bill').toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleFileChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (event) => {
        try {
          const result = importBills(event.target?.result as string);
          if (result.success) {
    ...
    <ModalPortal isOpen={isOpen} onClose={onClose}>
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white dark:bg-zinc-800 rounded-lg p-6 max-w-xl w-full max-h-[90vh] transition-colors flex flex-col">
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  className={`px-2 py-1 text-sm rounded-md ${
                    sortMethod === 'date' ? 'bg-blue-600 text-white dark:bg-blue-500' : 'bg-zinc-100 text-zinc-700 dark:bg-zinc-700 dark:text-zinc-300'
                  } transition-colors`}
                >
                  Date
                </button>
                <button
                  ...
                  className={`px-2 py-1 text-sm rounded-md ${
                    sortMethod === 'name' ? 'bg-blue-600 text-white dark:bg-blue-500' : 'bg-zinc-100 text-zinc-700 dark:bg-zinc-700 dark:text-zinc-300'
                  } transition-colors`}
                >
                  Name
                </button>
          ...
          <div className="mb-4 overflow-y-auto flex-grow" style={{ maxHeight }}>
            {filteredAndSortedBills.length > 0 ? (
              filteredAndSortedBills.map((bill) => (
                <BillHistoryItem key={bill.id} bill={bill} onLoad={handleLoadBill} onDelete={handleDeleteBill} />
              ))
            ) : searchTerm ? (
          ...
          {importError && <Alert type="error" className="mb-4">Import error: {importError}</Alert>}

          {importSuccess && <Alert type="success" className="mb-4">Bills imported successfully!</Alert>}
          ...
          <div className="flex justify-between mt-4">
            <div className="flex flex-row h-auto space-x-2">
              ...
              <Button variant="danger" onClick={handleDeleteAllBills} disabled={bills.length === 0}>
                Delete All
              </Button>
            </div>

            <Button variant="primary" onClick={handleExport} disabled={filteredAndSortedBills.length === 0}>
              Export All Bills
            </Button>
          </div>
        </div>
