...
## Frontend
- `src/Components/ScanReceiptButton.tsx` — clicking "Scan Receipt" jumps straight into a "Choose Method" modal (`ModeSelectionModal`: Choose from Gallery / Take Photo) — there's no separate "Upload Receipt" landing step first. Choosing a method sets `useCameraCapture` and programmatically clicks a hidden, always-mounted `<input type="file">` (with `capture="environment"` when Take Photo was chosen); selecting a file immediately loads it and opens the crop modal — no separate submit click, and boundary auto-detection is currently disabled (`detectReceiptBoundary` in `src/lib/receiptEnhance.ts` is a stub that always returns `null` — see [receipt-enhance.md](receipt-enhance.md)'s Notes), so this always starts from the full image's corners. A validation or load error (bad file type, >5MB, failed to load) surfaces in that same modal with "Try Again" (reopens Choose Method) and "Cancel". Once a photo is loaded, the modal shows **"Crop Receipt"**: the full-image starting boundary as draggable handles via `ReceiptBoundaryEditor` (`src/Components/ReceiptBoundaryEditor.tsx`), with a "Reset boundary" button to revert manual drags back to that starting quad. No enhancement preview is shown at this step — only the crop selection. Confirming ("Use This Crop") runs `enhanceReceiptFromImageAndQuad` (perspective crop → grayscale+contrast enhance → resize to ≤2048px), stores the result, marks the bill `scanStatus: 'processing'`, closes immediately, fires `scanBillReceipt` without awaiting it. Used from `ItemsInput.tsx` ([bill-editing.md](bill-editing.md)). Also self-opens on mount if it receives router nav-state `{ autoOpenScan: true }` (immediately replaced with no state, so back/forward doesn't re-trigger it) — used by `SessionHomePage.tsx`'s "Scan New Bill" action, which creates an empty bill then navigates straight into its Items step with the scan modal already open. That action also defaults the new bill's `paidByPersonId` to whichever session person `settingsStore`'s `autoAddSelf`/`selfName` matches (`src/lib/selfPerson.ts`'s `resolveSelfPersonId`) — i.e. if the app knows who "you" are, you become the default payer for a bill you scan.
