...
## Summary
The core 4-step wizard (People → Items → Assign → Summary) for building a
single bill: add participants, add line items (optionally via receipt scan),
assign items/splits to people, then view per-person totals. Backed by a
...
- `src/Components/PeopleInput.tsx` — Step 1, People.
- `src/Components/ItemsInput.tsx` — Step 2, Items; embeds `ScanReceiptButton`, `EditItemModal`, `BillTotalsSummary`.
- `src/Components/ItemAssignment.tsx` — Step 3, Assign; embeds `SplitTypeDrawer`, `PassAndSplitButton`.
- `src/Components/BillSummary.tsx` — Step 4, Summary; per-person totals, share text, print.
- `src/Components/EditItemModal.tsx` — add/edit item modal.
- `src/Components/EditPersonModal.tsx` — rename person modal.
...
## Related features
- [pass-and-split.md](pass-and-split.md) — alternate assignment flow launched from Step 3.
- [scan-receipt.md](scan-receipt.md) — populates Step 2 items from a photo.
