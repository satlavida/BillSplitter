...
- `server/internal/api/presence_handlers.go` — `POST .../presence/heartbeat`, `GET .../presence` (public list of online personIds, plus an `activeSince` map of each online personId's continuous-activity-start timestamp — RFC3339 — used by the frontend to gate renaming an active/claimed person).
