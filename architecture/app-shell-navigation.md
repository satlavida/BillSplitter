...
- `src/Components/RightPanel/` — right-side people/activity panel, mounted in `AppShell` alongside `Sidebar`. Renders nothing unless the current session (`sessionStore`'s `currentSessionId`) is live — there's no meaningful "live people"/"activity" to show otherwise.
  ...
  - **Desktop/tablet** (`lg:` breakpoint, 1024px+): `RightPanel` is collapsible here too, not always-visible — `AppShell`'s `isRightPanelOpen` state (default collapsed, persisted to localStorage under `rightPanelOpen`, same pattern as `sidebarOpen`) gates both `RightPanel`'s own `hidden`/`hidden lg:block` className and whether `AppShell` reserves layout space (`lg:mr-72`), so it doesn't leave a permanent gap when collapsed.
  - **Mobile** (below `lg:`): `MobileRightPanel.tsx` wraps the same `RightPanel` content in a slide-in drawer (mirrors `Sidebar.tsx`'s overlay/outside-click/Escape handling, on the opposite edge). Both the mobile drawer and the desktop column share the same `isRightPanelOpen` state and are toggled by the same `RightPanelToggleButton.tsx` in the header (only rendered once the session is live, at every breakpoint) — a small sibling to `HamburgerButton.tsx` rather than a generalized shared component, since the two aren't otherwise identical. `AppShell` keeps the two mobile panels mutually exclusive (opening one closes the other) so neither squeezes the content column to nothing.
  - **Header z-index**: the header's outer row deliberately has no `position`/`z-index` of its own, so its title/branding render *behind* an open Sidebar/RightPanel instead of visually overlapping them. `HamburgerButton` and `RightPanelToggleButton` are each wrapped in their own `relative z-30` `<div>` so they stay clickable above either panel's full-screen dimming overlay (`z-20`) even while a panel is open — without that per-button wrapper, the *other* panel's toggle button would be covered and unclickable while one panel was open.
