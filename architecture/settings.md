...
- `src/settingsStore.ts` — persisted zustand store for `autoAddSelf`/`selfName`, `showDetailedQuantitySplit` (beta opt-in for `DependentQuantitySplitInput.tsx` — see [bill-editing.md](bill-editing.md), off by default for every user), plus `completedOnboarding: Record<string, boolean>` and `completeOnboarding(id)` for tracking one-time onboarding flows by id.
